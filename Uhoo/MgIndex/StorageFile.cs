...
namespace UhooIndexer.MgIndex
{
    ...
    internal class StorageFile<T>
    {
        ...
        {
            Logging.WriteLog("Loading split files...", Logging.LogType.Information, Logging.LogCaller.StorageFile);
            ...
            Logging.WriteLog("Number of split files = " + _files.Count, Logging.LogType.Information, Logging.LogCaller.StorageFile);
        }
        ...
        private long InternalWriteData(StorageItem<T> meta, byte[] data, bool raw)
        {
            lock (_readlock)
            {
                ...
                if (raw == false)
                {
                    ...
                    byte[] metabytes = BinaryJson.ToBjson(meta, new BinaryJsonParameters { UseExtensions = false });
        ...
        private void CreateNewStorageFile()
        {
            ...
            Logging.WriteLog("Split limit reached = " + _datawrite.Length, Logging.LogType.Information, Logging.LogCaller.StorageFile);
            int i = _files.Count;
            // close files
            FlushClose(_datawrite);
            FlushClose(_dataread);
            ...
            _dataread = new FileStream(_filename, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
            Logging.WriteLog("New storage file created, count = " + _files.Count, Logging.LogType.Information, Logging.LogCaller.StorageFile);
        }
        ...
        private StorageItem<T> ReadMetaData(FileStream fs, out int metasize)
        {
            ...
            if (type > 0)
            {
                ...
                StorageItem<T> meta;
                if (type == 1)
                    meta = BinaryJson.ToObject<StorageItem<T>>(hdr);
                else
                {
